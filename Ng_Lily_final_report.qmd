...
It should be noted that there were originally three categories in the dataset, representing statuses of dropped out, currently enrolled, or graduated. I decided to simplify the predictive process to focus on students that either dropped out or graduated, emitting the currently enrolled status. This is because being currently enrolled means one still has the possibility to drop out or graduate, so ultimately, if I had more time, I would use the predictive model on those students to decide if they will graduate or not.
